...
const Navbar = () => {
  ...
  return (
    <nav
      className={`fixed top-4 left-1/2 -translate-x-1/2 w-[95%] max-w-7xl z-50 transition-all duration-500 rounded-full px-6 py-4 flex items-center justify-between ${
        scrolled
          ? 'bg-white/80 backdrop-blur-xl shadow-lg border border-primary/10'
    ...
    >
      ...
      <div className="flex items-center gap-2">
        <div
          className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xl ${
      ...
      <div className="hidden md:flex items-center gap-8">
        {navLinks.map((link) => (
          <a
            key={link.name}
            href={link.href}
            className={`font-medium text-sm transition-transform hover:-translate-y-0.5 ${
              scrolled ? 'text-dark hover:text-accent' : 'text-white/90 hover:text-white'
            }`}
          >
            {link.name}
          </a>
