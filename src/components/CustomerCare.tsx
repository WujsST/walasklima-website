...
const CustomerCare = () => {
  const philosophyContainer = useRef<HTMLElement>(null);
  const stackContainer = useRef<HTMLElement>(null);
      ...
      cards.forEach((card, index) => {
        ...
        if (!isLast) {
          ScrollTrigger.create({
            ...
            scrub: 0.5,
            animation: gsap.to(card as gsap.DOMTarget, {
              scale: 0.92,
              y: -30,
              opacity: 0,
  ...
  const protocolSteps = [
    {
      num: '01',
      title: 'Diagnoza & Analiza',
      desc: 'Precyzyjne zlokalizowanie nieszczelności i usterek z wykorzystaniem promieni UV i wywiadu technicznego.',
    },
    {
      num: '02',
      title: 'Czyszczenie & Ozonowanie',
      desc: 'Uderzenie generatorem O3 (30 000 mg/h) eliminujące do 99.9% roztoczy, pleśni i bakterii z parownika i przewodów.',
    },
    {
      num: '03',
      title: 'Kalibracja Czynnika',
      desc: 'Precyzyjne odzyskanie starego czynnika, próżnia i uzupełnienie idealnej dawki z dodatkiem oleju PAG do sprężarki.',
    },
  ];
      ...
      <section ref={philosophyContainer} className="relative py-32 px-6 lg:px-20 bg-primary overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto relative z-10 text-center">
          ...
          <h2 className="philo-text text-4xl md:text-6xl lg:text-7xl font-drama text-white italic leading-tight">
            My skupiamy się na Twoim <br />
            <span className="text-accent not-italic font-heading font-black">Zdrowiu</span> i{' '}
            <span className="text-accent not-italic font-heading font-black">Komforcie.</span>
          </h2>
        </div>
      </section>
          ...
          >
            <div className="w-full max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-12 lg:gap-24">
                  ...
                  <div className="w-full h-64 flex flex-col justify-between p-4">
                    {[1, 2, 3, 4, 5].map((i) => (
                      <div key={i} className="w-full h-2 bg-primary/10 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-accent rounded-full w-0 animate-[pulse_2s_ease-in-out_infinite]"
                          style={{ width: `${Math.random() * 80 + 20}%`, animationDelay: `${i * 0.2}s` }}
                        ></div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="flex-1 space-y-6">
                ...
                <p className="font-sans text-lg text-gray-500 leading-relaxed max-w-lg">{step.desc}</p>
              </div>
            </div>
          </div>
        ))}
