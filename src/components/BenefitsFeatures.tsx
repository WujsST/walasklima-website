  ...
  const [cards, setCards] = useState<Card[]>([
    {
      ...
      desc: 'Idealna temperatura niezależnie od upałów na zewnątrz.',
      icon: <Thermometer size={20} className="text-accent" />,
    },
    {
      id: 2,
      title: 'Czyste Powietrze',
      desc: 'Filtry zatrzymujące kurz, pył, bakterie i roztocza.',
      icon: <Wind size={20} className="text-secondary" />,
    },
    {
      id: 3,
      title: 'Funkcja Grzania',
      desc: 'Wydajna pompa ciepła w zimowe dni, tańsza niż prąd.',
...
const TelemetryTypewriter = () => {
  ...
  return (
    <div
      ref={container}
      className="bg-dark rounded-[2rem] p-8 shadow-xl flex flex-col relative overflow-hidden h-[340px]"
    >
...
const CursorScheduler = () => {
  ...
  return (
    <div
      ref={container}
      className="bg-white rounded-[2rem] p-8 shadow-xl border border-gray-100 flex flex-col relative overflow-hidden h-[340px]"
    >
      ...
      <div className="relative w-full h-full flex flex-col justify-between">
        <div className="grid grid-cols-7 gap-1">
          {days.map((day, i) => (
            <div key={i} className="text-center text-xs font-mono text-gray-400 mb-2">
              {day}
            </div>
          ))}
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="font-mono text-gray-500 max-w-2xl mx-auto">
            Dlaczego warto polegać na certyfikowanym serwisie? Od powietrza, którym oddychasz, po mikroklimat Twojego
            domu.
          </p>
        </div>
