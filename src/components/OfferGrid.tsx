      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {offers.map((offer) => (
            <div
              key={offer.id}
              className={`relative bg-white rounded-3xl p-8 flex flex-col transition-transform hover:-translate-y-2 duration-300 shadow-xl shadow-primary/5 ${
                offer.popular ? 'border-2 border-accent' : 'border border-gray-100'
              }`}
            >
              {offer.popular && (
              ...
              <div
                className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 ${
                  offer.popular ? 'bg-primary text-white' : 'bg-soft text-primary'
                }`}
              >
                {offer.icon}
              </div>
              ...
              <a
                href="#kontakt"
                className={`mt-auto w-full py-3 rounded-xl font-bold text-sm text-center transition-colors ${
                  offer.popular ? 'bg-primary text-white hover:bg-primary/90' : 'bg-soft text-primary hover:bg-gray-200'
                }`}
              >
                Zamów Usługę
              </a>
