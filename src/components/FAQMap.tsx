...
const FAQMap = () => {
  ...
  return (
    <section id="faq" className="py-24 px-6 lg:px-20 bg-background flex flex-col items-center">
      ...
      <div
        id="kontakt"
        className="w-full max-w-7xl bg-primary rounded-[3rem] p-12 lg:p-20 relative overflow-hidden mb-24 flex flex-col lg:flex-row items-center justify-between gap-12 shadow-2xl"
      >
        ...
        <div className="relative z-10 lg:max-w-xl">
          <h2 className="font-heading font-black text-4xl lg:text-5xl text-white mb-6">
            Gotowy na idealną temperaturę?
          </h2>
          <p className="text-white/80 font-sans text-lg mb-8">
            Zadzwoń do nas lub zamów błyskawiczny audyt online. Oddzwonimy w ciągu 15 minut z gotową propozycją.
          </p>
        </div>
        <div className="relative z-10 flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto">
          <a
            href="tel:+48604099876"
            className="w-full sm:w-auto bg-white text-primary px-8 py-4 rounded-xl font-bold text-center hover:bg-soft transition-colors shadow-lg"
          >
            Zadzwoń: 604 099 876
          </a>
          <Link
            href="/quiz"
            className="w-full sm:w-auto bg-accent text-white px-8 py-4 rounded-xl font-bold text-center hover:bg-[#549E39] transition-colors shadow-lg shadow-accent/20"
          >
            Formularz Online
          </Link>
        </div>
            ...
            {faqs.map((faq, i) => (
              <div
                ...
                className={`border border-gray-100 rounded-2xl overflow-hidden transition-all duration-300 ${
                  openIndex === i ? 'bg-white shadow-md' : 'bg-transparent hover:bg-white'
                }`}
              >
                <button
                  onClick={() => toggleFaq(i)}
                ...
                >
                  <span className={`font-bold font-heading text-lg ${openIndex === i ? 'text-primary' : 'text-dark'}`}>
                    {faq.q}
                  </span>
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${
                      openIndex === i ? 'bg-primary/10 text-primary' : 'bg-gray-100 text-gray-400'
                    }`}
                  >
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-xl border border-gray-100 flex-1 flex flex-col relative overflow-hidden">
            <div className="flex flex-col sm:flex-row gap-6 mb-6 z-10 relative">
              <div className="flex items-start gap-3">
                <div className="p-3 bg-soft text-primary rounded-xl">
                  <MapPin size={20} />
                </div>
                <div>
              ...
              <div className="flex items-start gap-3">
                <div className="p-3 bg-soft text-accent rounded-xl">
                  <Clock size={20} />
                </div>
                <div>
